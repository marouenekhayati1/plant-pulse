/**
 * WattNow integration: AWS Cognito SRP login + realtime device polling.
...
async function getValidSession(forceRefresh = false): Promise<Session> {
  if (!forceRefresh) {
...
async function fetchLastValues(session: Session): Promise<RawDevice[] | { unauthorized: true }> {
...
export async function pollWattNow(): Promise<RealtimeSnapshot> {
  try {
    ...
    if ("unauthorized" in raw) {
      ...
      if ("unauthorized" in raw) {
        return lastSnapshotFallback("WattNow refused even after re-login");
      }
    }
    ...
    return lastSnapshotFallback(msg);
  }
}
